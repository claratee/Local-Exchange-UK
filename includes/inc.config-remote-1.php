...
if (!isset($globals) && $running_upgrade_script!=true)
{
...
$globals['SERVER_DOMAIN'] = "https://www.camlets.org.uk";	// CT update - eg https://yoursite.co.uk. to remove all the inline instances and allow you to work with ssl. 

// What is the path to the site? This is null for many sites.
$globals['SERVER_PATH_URL'] = "/members";	// CT update - like /members - no ending slash

// The following only needs to be set if Pear has been
// installed manually by downloading the files
$globals['PEAR_PATH']=""; // no ending slash
...
$globals['UPLOADS_PATH']=$_SERVER["DOCUMENT_ROOT"].$globals['SERVER_PATH_URL']."/uploads/";
$globals['VENDOR_PATH']=$_SERVER["DOCUMENT_ROOT"].$globals['SERVER_PATH_URL']."/vendor/"; //CT added extra libraries place
$globals['INCLUDES_PATH'] = $_SERVER["DOCUMENT_ROOT"].$globals['SERVER_PATH_URL']."/includes/";
$globals['TEMPLATES_PATH'] = $_SERVER["DOCUMENT_ROOT"].$globals['SERVER_PATH_URL']."/includes/templates/";

//CT stuff for viewing site in browser
$globals['HTTP_BASE'] = $globals['SERVER_DOMAIN'] . $globals['SERVER_PATH_URL'];
$globals['IMAGES_PATH'] = $globals['HTTP_BASE'] . "/images/";
$globals['STYLES_PATH'] = $globals['HTTP_BASE'] . "/styles/";  //CT CSS
$globals['SCRIPTS_PATH'] = $globals['HTTP_BASE'] . "/scripts/"; //CT Javascript
...
define ("PHONE_ADMIN","07709 588384"); // an email address may be substituted...
...
if (DEFAULT_COUNTRY == "United Kingdom" && DEFAULT_COUNTRY=='London') {
    ...
    define ("ADDRESS_LINE_3", "Borough"); //CT not used?
