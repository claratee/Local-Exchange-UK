...
class cCategory
{
   ...
   /**
	...
	function SaveNewCategory() {
		...
		$insert = $cDB->Query("INSERT INTO " .DATABASE_CATEGOTIES . "(parent_id, description) VALUES (". $cDB->EscTxt($this->getParentCategoryId()) .", ". $cDB->EscTxt($this->getCategoryName()) .");");
	...
	function SaveCategory() {
		...
		$update = $cDB->Query("UPDATE " .DATABASE_CATEGOTIES . " SET parent_id=". $cDB->EscTxt($this->getParentCategoryId()) .", description=". $cDB->EscTxt($this->getCategoryName()) ." WHERE category_id=". $cDB->EscTxt($this->getCategoryId()) .";");
	...
	function DeleteCategory() {
		...
		$delete = $cDB->Query("DELETE FROM " .DATABASE_CATEGOTIES . " WHERE category_id=". $cDB->EscTxt($this->getCategoryId()));
		
		if(mysqli_affected_rows() == 1) {
			//unset($this);	
			return true;
		} else {
			$cStatusMessage->Error("Could not delete category code '".$this->getCategoryId()."'.");
...
*/
		/* 
		if($active_only) {
			$query = $cDB->Query("SELECT DISTINCT ".DATABASE_CATEGORIES.".category_id, ".DATABASE_CATEGORIES.".description FROM ".DATABASE_CATEGORIES.", ".DATABASE_LISTINGS." WHERE ".DATABASE_LISTINGS.".category_id =".DATABASE_CATEGORIES.".category_id AND status='". ACTIVE ."' AND type LIKE ". $cDB->EscTxt($type) ." ORDER BY ". DATABASE_CATEGORIES .".description;");
		} else {
...
/*		
		...
		if($i == 0) {
			if ($redirect) {
				$cStatusMessage->Error("Error accessing a category record.");
				//include("redirect.php");			
			} else {
				return false;
			}
