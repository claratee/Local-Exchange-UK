...
class cListing
{
	...
	function Load($condition, $order_by="listing_id ASC")
	{
		...
		if($values = $cDB->FetchArray($query)) {
            ...
            $cStatusMessage->Error("There was an error accessing the listing  {$listing_id}");	
            return false;
		}
	...
	function Build($values)
	{
		...
		//$query = $cDB->Query("SELECT description, category_id, member_id, rate, status, posting_date, expire_date, reactivate_date FROM ".DATABASE_LISTINGS." WHERE title=".$cDB->EscTxt($title)." AND member_id=" . $cDB->EscTxt($member_id) . " AND type=". $cDB->EscTxt($type_code) .";");
